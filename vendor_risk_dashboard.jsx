...
import {
  ...
  BarChart2, Zap, TrendingDown, Award, Clock
} from "lucide-react";
import {
...
const riskDist = [
  { name: "Low Risk",    value: 280, color: "#16A34A" },
  { name: "Medium Risk", value: 150, color: "#F59E0B" },
  { name: "High Risk",   value: 70,  color: "#DC2626" },
];

const topRisk = [
  { id:1, name:"Apex Manufacturing",  risk:"High",   score:92, cat:"Manufacturing", trend:"+5"  },
  { id:2, name:"Vertex Supplies Co.", risk:"High",   score:88, cat:"Logistics",     trend:"+2"  },
  { id:3, name:"Orion Tech Parts",    risk:"High",   score:81, cat:"Technology",    trend:"-3"  },
  { id:4, name:"Global Steel Corp",   risk:"High",   score:78, cat:"Materials",     trend:"+8"  },
  { id:5, name:"Pacific Logistics",   risk:"Medium", score:67, cat:"Logistics",     trend:"+12" },
];

const alerts = [
  { id:1, vendor:"Apex Manufacturing",  msg:"Defect rate exceeded 12% threshold",       type:"danger",  time:"2h ago" },
  { id:2, vendor:"Vertex Supplies Co.", msg:"SLA breach — delivery delayed 4 days",      type:"danger",  time:"5h ago" },
  { id:3, vendor:"Pacific Logistics",   msg:"Compliance certificate expiring in 7 days", type:"warning", time:"1d ago" },
  { id:4, vendor:"Northern Fab Ltd.",   msg:"Financial score dropped below threshold",   type:"warning", time:"2d ago" },
];

const vendors = [
  { id:1,  name:"Apex Manufacturing",  risk:"High",   score:92, cat:"Manufacturing", comp:"Non-Compliant", ctrt:"Active"       },
  { id:2,  name:"Vertex Supplies Co.", risk:"High",   score:88, cat:"Logistics",     comp:"Non-Compliant", ctrt:"Active"       },
  { id:3,  name:"Orion Tech Parts",    risk:"High",   score:81, cat:"Technology",    comp:"At Risk",       ctrt:"Active"       },
  { id:4,  name:"Global Steel Corp",   risk:"High",   score:78, cat:"Materials",     comp:"At Risk",       ctrt:"Under Review" },
  { id:5,  name:"Pacific Logistics",   risk:"Medium", score:67, cat:"Logistics",     comp:"At Risk",       ctrt:"Active"       },
  { id:6,  name:"Northern Fab Ltd.",   risk:"Medium", score:61, cat:"Manufacturing", comp:"Compliant",     ctrt:"Active"       },
  { id:7,  name:"TechCore Systems",    risk:"Medium", score:55, cat:"Technology",    comp:"Compliant",     ctrt:"Active"       },
  { id:8,  name:"SwiftShip Co.",       risk:"Medium", score:48, cat:"Logistics",     comp:"Compliant",     ctrt:"Active"       },
  { id:9,  name:"Reliable Parts Inc.", risk:"Low",    score:24, cat:"Manufacturing", comp:"Compliant",     ctrt:"Active"       },
  { id:10, name:"SwiftShip Logistics", risk:"Low",    score:18, cat:"Logistics",     comp:"Compliant",     ctrt:"Active"       },
  { id:11, name:"Precision Metals",    risk:"Low",    score:12, cat:"Materials",     comp:"Compliant",     ctrt:"Active"       },
  { id:12, name:"EcoSupply Solutions", risk:"Low",    score:9,  cat:"Services",      comp:"Compliant",     ctrt:"Active"       },
];
...
function RiskBadge({ level, large }) {
  return (
    <span style={{
      background:RISK_BG[level], color:RISK_HEX[level],
      border:`1px solid ${RISK_BD[level]}`,
      fontSize:large?"15px":"10px", fontWeight:700,
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"20px" }}>
      <div style={{ display:"flex", gap:"16px" }}>
        <KPICard title="Total Vendors" value={500} Icon={Building2}     color="#2563EB" note="Across all categories"     />
        <KPICard title="Low Risk"      value={280} Icon={CheckCircle}   color="#16A34A" note="56% of suppliers"          />
        <KPICard title="Medium Risk"   value={150} Icon={AlertTriangle} color="#D97706" note="30% of suppliers"          />
        <KPICard title="High Risk"     value={70}  Icon={Shield}        color="#DC2626" note="Requires immediate action" />
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"320px 1fr", gap:"16px" }}>
          ...
          <div style={{ height:"230px" }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Legend iconType="circle" iconSize={8} formatter={v => <span style={{ fontSize:"12px", color:"#374151", fontWeight:500 }}>{v}</span>} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div style={{ ...card, padding:"24px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"18px" }}>
            ...
            <span style={{ background:"#FEF2F2", color:"#DC2626", fontSize:"10px", fontWeight:700, padding:"4px 10px", borderRadius:"20px", border:"1px solid #FECACA" }}>
              {topRisk.filter(v=>v.risk==="High").length} CRITICAL
            </span>
          </div>
            ...
            <tbody>
              {topRisk.map(v => (
                <tr key={v.id}>
                  ...
                  <td style={{ padding:"11px 0", fontSize:"12px", fontWeight:700, color:v.trend.startsWith("+")?"#DC2626":"#16A34A", fontFamily:"'IBM Plex Mono',monospace", borderBottom:"1px solid #F9FAFB" }}>{v.trend}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div>
      ...
      <div style={{ display:"flex", gap:"12px", marginBottom:"24px", flexWrap:"wrap" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", background:"#fff", borderRadius:"10px", padding:"10px 16px", flex:1, minWidth:"200px", boxShadow:"0 1px 3px rgba(0,0,0,0.06)", border:"1px solid #F3F4F6" }}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search vendors by name..." style={{ border:"none", outline:"none", background:"transparent", fontSize:"13px", color:"#111827", flex:1, fontFamily:"inherit" }} />
        </div>
        {["All","High","Medium","Low"].map(f => (
          <button key={f} onClick={()=>setFilter(f)} style={{ padding:"10px 18px", borderRadius:"10px", border:`2px solid ${filter===f?(f==="All"?"#2563EB":RISK_HEX[f]):"#E5E7EB"}`, background:filter===f?(f==="All"?"#EFF6FF":RISK_BG[f]):"#fff", color:filter===f?(f==="All"?"#2563EB":RISK_HEX[f]):"#6B7280", fontSize:"13px", fontWeight:600, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" }}>
            {f}
          </button>
        ))}
      </div>
      {shown.length===0 ? (
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"16px" }}>
          {shown.map(v => (
            <div key={v.id}
              style={{ ...card, padding:"20px", borderLeft:`4px solid ${RISK_HEX[v.risk]}`, cursor:"pointer", transition:"transform 0.15s,box-shadow 0.15s" }}
            ...
            >
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"14px" }}>
                <div style={{ width:"40px", height:"40px", borderRadius:"10px", background:`${RISK_HEX[v.risk]}15`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <Building2 size={18} color={RISK_HEX[v.risk]} />
                </div>
                <RiskBadge level={v.risk} />
              </div>
              <p style={{ fontSize:"14px", fontWeight:700, color:"#111827", marginBottom:"2px" }}>{v.name}</p>
              <p style={{ fontSize:"12px", color:"#9CA3AF", marginBottom:"16px" }}>{v.cat}</p>
              <div style={{ borderTop:"1px solid #F3F4F6", paddingTop:"14px", display:"flex", justifyContent:"space-between" }}>
                <div>
                  <p style={{ ...lbl, marginBottom:"4px" }}>SCORE</p>
                  <p style={{ fontSize:"22px", fontWeight:800, color:RISK_HEX[v.risk], fontFamily:"'IBM Plex Mono',monospace" }}>{v.score}</p>
                </div>
                <div>
                  <p style={{ ...lbl, marginBottom:"4px" }}>COMPLIANCE</p>
                  <p style={{ fontSize:"12px", fontWeight:600, color:compColor(v.comp) }}>{v.comp}</p>
                </div>
                <div>
                  <p style={{ ...lbl, marginBottom:"4px" }}>CONTRACT</p>
                  <p style={{ fontSize:"12px", fontWeight:600, color:"#374151" }}>{v.ctrt}</p>
                </div>
              </div>
            </div>
          ))}
...
function PredictPage() {
  const [form, setForm] = useState({ defectRate:5, delayRate:10, compIssues:1, finScore:75, respTime:2 });
  ...
  const predict = () => {
    ...
    setTimeout(() => {
      const { defectRate, delayRate, compIssues, finScore, respTime } = form;
      const raw = (defectRate/25)*34+(delayRate/50)*28+(compIssues/10)*19+((100-finScore)/100)*12+((respTime-1)/9)*7;
      const score = Math.round(Math.min(100,Math.max(0,raw)));
      const level = score>=60?"High":score>=35?"Medium":"Low";
      const conf  = score>75||score<20?93:score>60||score<30?86:78;
      const drivers = [];
      if (defectRate>10) drivers.push(`High defect rate (${defectRate}%)`);
      if (delayRate>20)  drivers.push(`Frequent delivery delays (${delayRate}%)`);
      if (compIssues>=3) drivers.push(`Multiple compliance violations (${compIssues})`);
      if (finScore<50)   drivers.push(`Low financial health score (${finScore})`);
      if (respTime>4)    drivers.push(`Slow response time (${respTime} days avg.)`);
      if (!drivers.length) drivers.push("No major risk drivers identified");
      const recs = level==="High"
        ? ["Conduct immediate supplier audit","Review and renegotiate SLA terms","Increase monitoring to weekly cadence","Source backup supplier as contingency"]
        : level==="Medium"
        ? ["Schedule quarterly performance review","Request updated compliance documentation","Configure automated performance alerts"]
        : ["Maintain standard monitoring schedule","Flag as candidate for preferred supplier status"];
      setResult({ level, score, conf, drivers, recs });
      setLoading(false);
    }, 1600);
  };
    ...
    return (
      <div style={{ marginBottom:"22px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"5px" }}>
          ...
          <span style={{ fontSize:"14px", fontWeight:700, color:"#2563EB", fontFamily:"'IBM Plex Mono',monospace" }}>{form[field]}{unit}</span>
        </div>
        {desc && <p style={{ fontSize:"11px", color:"#9CA3AF", marginBottom:"7px" }}>{desc}</p>}
        <input type="range" min={min} max={max} value={form[field]} onChange={e=>setForm(p=>({...p,[field]:Number(e.target.value)}))} style={{ width:"100%", accentColor:"#2563EB" }} />
  ...
  return (
    <div>
      <div style={{ marginBottom:"24px" }}>
        ...
        <p style={{ fontSize:"13px", color:"#9CA3AF", marginTop:"4px" }}>Predict vendor risk using ML-driven intelligence</p>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"24px", alignItems:"start" }}>
        <div style={{ ...card, padding:"28px" }}>
          ...
          <button onClick={predict} disabled={loading} style={{ width:"100%", padding:"14px", background:loading?"#93C5FD":"#2563EB", color:"#fff", border:"none", borderRadius:"10px", fontSize:"15px", fontWeight:700, cursor:loading?"not-allowed":"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", transition:"background 0.2s", marginTop:"4px" }}>
            <Zap size={16} />
        ...
        <div>
          {!result && !loading && (
            <div style={{ ...card, padding:"80px", textAlign:"center", display:"flex", flexDirection:"column", alignItems:"center" }}>
              ...
              <p style={{ fontSize:"12px", color:"#9CA3AF", marginTop:"8px", maxWidth:"220px", lineHeight:1.6 }}>Configure vendor metrics and click Predict Risk to begin</p>
            </div>
          )}
          {loading && (
            <div style={{ ...card, padding:"80px", textAlign:"center", display:"flex", flexDirection:"column", alignItems:"center" }}>
          ...
          {result && (
            <div style={{ display:"flex", flexDirection:"column", gap:"14px" }}>
              <div style={{ ...card, padding:"28px", background:RISK_BG[result.level], border:`2px solid ${RISK_BD[result.level]}`, textAlign:"center" }}>
                ...
                <div style={{ display:"flex", justifyContent:"center", gap:"48px", marginTop:"22px" }}>
                  <div>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:"20px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1.3fr 1fr", gap:"16px" }}>
        ...
        <div style={{ ...card, padding:"24px" }}>
          ...
          {[
            { rank:1, name:"EcoSupply Solutions", score:9,  cat:"Services",      medal:"#F59E0B" },
            { rank:2, name:"Precision Metals",    score:12, cat:"Materials",     medal:"#94A3B8" },
            { rank:3, name:"SwiftShip Logistics", score:18, cat:"Logistics",     medal:"#CD7C2F" },
            { rank:4, name:"Reliable Parts Inc.", score:24, cat:"Manufacturing", medal:null       },
            { rank:5, name:"SwiftShip Co.",       score:48, cat:"Logistics",     medal:null       },
          ].map(v => (
      ...
      <div style={{ ...card, padding:"24px" }}>
        ...
        <div style={{ height:"240px" }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={catBreak} margin={{ left:-20, right:10, top:0, bottom:0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F3F4F6" />
...
function ReportsPage() {
  const reports = [
    { title:"Vendor Risk Report",      desc:"Comprehensive risk scores for all 500 vendors with trend analysis",  Icon:Shield,       color:"#DC2626", updated:"Today, 09:00 AM" },
    ...
    { title:"Supplier Ranking Report", desc:"Full vendor performance leaderboard ranked by composite risk score",  Icon:Award,        color:"#F59E0B", updated:"Yesterday"        },
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"16px" }}>
        {reports.map((r,i) => (
          <div key={i}
            style={{ ...card, padding:"24px", cursor:"pointer", transition:"transform 0.15s,box-shadow 0.15s", display:"flex", flexDirection:"column" }}
            ...
            <div style={{ borderTop:"1px solid #F3F4F6", marginTop:"16px", paddingTop:"14px" }}>
              ...
              <div style={{ display:"flex", gap:"8px" }}>
                {["CSV","Excel","PDF"].map(fmt => (
                  <button key={fmt} onClick={e=>e.stopPropagation()} style={{ padding:"5px 11px", borderRadius:"6px", border:"1px solid #E5E7EB", background:"#fff", color:"#374151", fontSize:"11px", fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", gap:"4px", fontFamily:"inherit" }}>
                    <Download size={9} /> {fmt}
                  </button>
                ))}
              </div>
    ...
    <div>
      ...
      <div style={{ maxWidth:"680px", display:"flex", flexDirection:"column", gap:"20px" }}>
        <div style={{ ...card, padding:"24px" }}>
          ...
          {[{l:"Defect Rate Alert Threshold",v:"12%"},{l:"Delay Rate Alert Threshold",v:"20%"},{l:"Compliance Issue Alert Level",v:"3"},{l:"Financial Score Warning Level",v:"50"}].map((s,i,arr) => (
        ...
        <div style={{ ...card, padding:"24px" }}>
          ...
          {[{key:"email",l:"Email alerts for High Risk vendors"},{key:"weekly",l:"Weekly summary reports"},{key:"realtime",l:"Real-time SLA breach notifications"}].map((s,i,arr) => (
            <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingBottom:"14px", borderBottom:i<arr.length-1?"1px solid #F3F4F6":"none", marginBottom:i<arr.length-1?"14px":0 }}>
              <p style={{ fontSize:"13px", color:"#374151", fontWeight:500 }}>{s.l}</p>
        ...
        <div style={{ ...card, padding:"24px" }}>
          ...
          <p style={{ fontSize:"12px", color:"#9CA3AF", marginBottom:"16px" }}>Connect real datasets and ML models when available</p>
          {[
            {l:"Vendor Dataset Source",v:"Mock Data → Replace with cleaned_vendor_data.csv"},
            {l:"ML Model",v:"Placeholder → Connect vendor_model.pkl"},
            {l:"Feature Importance",v:"Static → Replace with model.feature_importances_"},
          ].map((s,i,arr) => (
...
export default function App() {
  ...
  const titles = { dashboard:"Dashboard", vendors:"Vendor Explorer", predict:"Risk Prediction", analytics:"Analytics", reports:"Reports", settings:"Settings" };
      ...
      <aside style={{ width:"220px", height:"100vh", background:"linear-gradient(180deg,#0D1421 0%,#111827 100%)", position:"fixed", left:0, top:0, display:"flex", flexDirection:"column", zIndex:100 }}>
        ...
        <nav style={{ flex:1, padding:"14px 12px", overflowY:"auto" }}>
          ...
          {nav.map(({ id, label, Icon }) => {
            ...
            return (
              <button key={id} onClick={()=>setPage(id)} style={{ display:"flex", alignItems:"center", gap:"10px", width:"100%", padding:"9px 12px", borderRadius:"8px", border:"none", background:active?"#2563EB18":"transparent", color:active?"#60A5FA":"#6B7280", fontSize:"13px", fontWeight:active?600:400, cursor:"pointer", marginBottom:"2px", borderLeft:`3px solid ${active?"#2563EB":"transparent"}`, transition:"all 0.15s", textAlign:"left", fontFamily:"inherit" }}>
                <Icon size={15} />
                {label}
              </button>
            );
      ...
      <div style={{ marginLeft:"220px", height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <header style={{ height:"56px", background:"#fff", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 32px", boxShadow:"0 1px 0 #E5E7EB", zIndex:50 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"20px" }}>
            <span style={{ fontSize:"12px", color:"#9CA3AF" }}>June 2026</span>
            <div style={{ position:"relative" }}>
              ...
              <div style={{ position:"absolute", top:"-4px", right:"-4px", width:"14px", height:"14px", background:"#DC2626", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <span style={{ fontSize:"8px", color:"#fff", fontWeight:700 }}>4</span>
              </div>
            </div>
          </div>
        </header>
